// Worker that SKIPS importScripts entirely.
// importScripts(crossOriginCDNUrl) blocks the Worker's event loop under
// COEP:require-corp in WebKit. Instead, we go straight to dynamic import().
...
const load = async ({ coreURL: _coreURL, wasmURL: _wasmURL, workerURL: _workerURL }) => {
  const first = !ffmpeg;
  // SKIP importScripts — it hangs under COEP in WebKit classic Workers.
  // Go straight to dynamic import(). This is safe because the file is an
  // ESM module that must be loaded via import(), not importScripts().
